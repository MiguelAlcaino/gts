    ...
    implements Constants
{
        ...
        throws ServletException, IOException
    {
    	...
    	//if(pageName.equals("temp_user_login")){
    	//	String url_custom = "/jsp/prueba.jsp";
    	//	request.setAttribute("miguel","alcaino");
    	//	request.getRequestDispatcher(url_custom).include(request, response);
    ...
    private void _doWork_wrapper(boolean isPost, HttpServletRequest request, HttpServletResponse response)
        throws ServletException, IOException
    {
    	if(TempUserController.paginas.containsKey(request.getParameter("page"))){
    		TempUserController tuc = new TempUserController(request, response);
    		tuc.executePage();
    		return;
    	}
        ...
        throws ServletException, IOException
    {
        ...
        if (!isSecure && privLabel.getBooleanProperty(PrivateLabel.PROP_Track_forwardToSecureAccess,false)) {
            if (StringTools.startsWithIgnoreCase(requestURL,"http:")) {
                ...
                //out.println("<META HTTP-EQUIV=\"refresh\" CONTENT=\"1; URL=http://winksoftware.cl\"></META>");
                out.println("</HTML>");
                out.close();
                return;
            } else {
        ...
        if (trackPage == null) {
            ...
            if (!wasLoggedIn && (user != null) && user.hasFirstLoginPageID()) {
                ...
                pageName = PAGE_MENU_TOP; // TODO: runtime config
            	//pageName="miguel";
            }
        ...
        throws IOException
    {
        ...
        if ((loginPage instanceof AccountLogin) && ((AccountLogin)loginPage).hasCustomLoginURL()) {
            ...
            //out.println("<META HTTP-EQUIV=\"refresh\" CONTENT=\"1; URL=http://www.ogb.cl\"></META>");
            out.println("</HTML>");
            out.close();
        } else
        if (loginPage != null) {
