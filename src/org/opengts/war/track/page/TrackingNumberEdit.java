...
public class TrackingNumberEdit
    ...
    implements Constants
{
    ...
    public TrackingNumberEdit()
    {
        this.setBaseURI(RequestProperties.TRACK_BASE_URI());
        this.setPageName(PAGE_TRACKING_NUMBER_EDIT);
        ...
        HTMLOutput HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
            public void write(PrintWriter out) throws IOException {
            	...
            	if (reqState.getCommandName().equals("edit_temp_user")) {
            		String submit = AttributeTools.getRequestString(request, "submit_edit_temp_user", "");
            		 if (SubmitMatch(submit,"Guardar" )) {
            			 try{
            			 int tracking_number_id = AttributeTools.getRequestInt(request, "tracking_number_id", 0);
            			 
            			 TrackingNumber tn = TrackingNumber.getTrackingNumberById(tracking_number_id);
            			 TempUser tu = TempUser.getTempUserByID(tn.getTempUserID());
        				 ...
        				 tn.update();
            				 ...
            				 Track.writeMessageResponse(reqState, "Registro actualizado correctamente");
                             return;
            	...
            	*/
            	...
            	int id_tracking_user = AttributeTools.getRequestInt(request, "id", 0);
            	
            	TrackingNumber tn = TrackingNumber.getTrackingNumberById(id_tracking_user);
            	TempUser tu = TempUser.getTempUserByID(tn.getTempUserID());
            	out.println("<span class='menuTitle'>Registro de n° de tracking</span>\n");
            	
            	out.println("<hr />\n");
            	out.println("<form method='post' action='./Track?page="+PAGE_TRACKING_NUMBER_EDIT+"&page_cmd=edit_temp_user&tracking_number_id="+tn.getTrackingNumberID()+"'>");
            		out.println("<table>");
            			out.println("<tbody>");
            				out.println("<tr>");
            					out.println("<td>");
            					...
            					out.println("</td>");
            					out.println("<td>");
            						out.println("<input value='"+tu.getNombre()+"' required='required' type='text' name='temp_user_nombre' />");
            					out.println("</td>");
    						out.println("</tr>");
    						out.println("<tr>");
	        					out.println("<td>");
	        					...
	        					out.println("</td>");
	        					out.println("<td>");
	        						out.println("<input value='"+tu.getApellido()+"' required='required' type='text' name='temp_user_apellido' />");
	        					out.println("</td>");
							out.println("</tr>");
							out.println("<tr>");
	        					out.println("<td>");
	        					...
	        					out.println("<td>");
	        	            		...
	        	            		while(it.hasNext()){
	        	            			String next = it.next();
	        	            			if(next == tn.getDeviceID()){
	        	            				out.println("<option selected value='"+next+"'>"+next+"</option>");
	        	            			}else{
							...
							out.println("<tr>");
	        					...
	        					out.println("</td>");
	        					out.println("<td>");
	        						out.println("<input value='"+tu.getEmail()+"' required='required' type='email' name='temp_user_email' />");
	        					out.println("</td>");
        					out.println("</tr>");
        					...
        					out.println("<tr>");
	        					...
	        					out.println("</td>");
	        					out.println("<td>");
	        						out.println("<input value='"+tn.getNumber()+"' required='required' type='text' name='tracking_number_number' value='"+number_generado+"' /> ");
	        					out.println("</td>");
							out.println("</tr>");
							out.println("<tr>");
							out.println("<tr>");
	        					...
	        					out.println("<td>");
	        						out.println("<select name='tracking_number_on_off' >");
	        							out.println("<option "+(tn.isOn_off() ? "selected" : "")+" value ='1'>Activado</option>");
	        							out.println("<option "+(tn.isOn_off() ? "" : "selected")+" value ='0'>Desactivado</option>");
	        						out.println("</select>");
	        					out.println("</td>");
							out.println("</tr>");
							out.println("<tr>");
	        					...
	        					out.println("</td>");
	        					out.println("<td>");
	        						out.println("<input value='"+tn.getTempPassword()+"' id='tracking_number_password' required='required' type='password' name='tracking_number_password' /><a href='#' id='generar_password_button'>Generar password</a> <div id='password_generada'></div>");
	        					out.println("</td>");
							out.println("</tr>");
        					...
        					out.println("</td>");
        					out.println("<td>");
        						out.println("<input value='("+tn.getStartLatitude()+", "+tn.getStartLongitude()+")' id='start_coordinate' value='' readonly='readonly' class=\"tooltip\"  type='text' name='start_coordinate' />");
        						out.println("<input value='"+tn.getStartLatitude()+"' class='lat' type='hidden' value='' name='tracking_number_start_latitude' />");
        						out.println("<input value='"+tn.getStartLongitude()+"' class='lng' type='hidden' value='' name='tracking_number_start_longitude' />");
        					out.println("</td>");
						out.println("</tr>");
						/*
						out.println("<tr>");
						...
						*/
						out.println("<tr>");
        					...
        					out.println("</td>");
        					out.println("<td>");
        						out.println("<input value='("+tn.getEndLatitude()+", "+tn.getEndLongitude()+")' id='end_coordinate' value='' readonly='readonly' class=\"tooltip\"  type='text' name='end_coordinate' />");
        						out.println("<input value='"+tn.getEndLatitude()+"' class='lat' type='hidden' value='' name='tracking_number_end_latitude' />");
        						out.println("<input value='"+tn.getEndLongitude()+"' class='lng' type='hidden' value='' name='tracking_number_end_longitude' />");
        					out.println("</td>");
						out.println("</tr>");
						/*
						out.println("<tr>");
						...
						*/
						...
						out.println("<tr>");
        					...
        					out.println("</td>");
        					out.println("<td>");
        						out.println("<input value='"+sdf.format(tn.getStartTime()) +"' type='text' id='tracking_number_start_time' name='tracking_number_start_time' readonly='readonly'  />");
        					out.println("</td>");
						out.println("</tr>");
						out.println("<tr>");
        					out.println("<td>");
        					...
        					out.println("</td>");
        					out.println("<td>");
        						out.println("<input value='"+sdf.format(tn.getEndTime()) +"' type='text' id='tracking_number_end_time' name='tracking_number_end_time' readonly='readonly' />");
        					out.println("</td>");
						out.println("</tr>");
            	...
            	out.println("	var map;\n"
            			...
            			+ "}else{"
            			+ "	var initial_position = new google.maps.LatLng(element.next('.lat').val(), element.next('.lat').next().val());\n"
            			+ "}"
            			+ "		var mapOptions = {\n"
            			+ "			zoom: 16,\n"
